# Codex Dynamic Meteor

本地代理网关，让 Codex 通过 `/model opus|sonnet|haiku` 动态路由到不同 LLM 厂商，支持 Anthropic/OpenAI 双协议格式双向转换。
...
```
Codex → ANTHROPIC_BASE_URL=http://127.0.0.1:9876 → axum proxy
  → model 带边界关键词匹配 → 确定厂商
...
│   ├── Codex/         # settings.json 读写 (一键对接/还原)
│   ├── commands/       # Tauri Commands: server/provider/log/stats/Codex
│   ├── db/             # SQLite: logs/migration (Arc<Mutex<Connection>>, 读时聚合)
│   └── monitor/        # Channel 实时推送 + SQLite 写入协调
├── src/
│   ├── components/     # layout/dashboard/providers/logs/Codex/settings/shared/ui
